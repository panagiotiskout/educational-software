...
if ($conn->connect_error) {
    ...
    $stmt = $conn->prepare("UPDATE userslog SET quiz4 = ? WHERE username = ?");
    if ($stmt === false) {
    ...
    $stmtQ = $conn->prepare("SELECT quiz1, quiz2 , quiz3 FROM userslog WHERE username = ?");
    if ($stmtQ === false) {
    ...
    //stmt1 checks if mo of user is null
                ...
                if ($stmt1->num_rows > 0) {
                    ...
                    $stmt2 = $conn->prepare("UPDATE userslog SET mo = COALESCE(($q1 + $q2 + $q3 + ?) / 4, ?) WHERE username = ?");
                    if($stmt2===false){
