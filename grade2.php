...
if ($conn->connect_error) {
    ...
    $stmt = $conn->prepare("UPDATE userslog SET quiz2 = ? WHERE username = ?");
    if ($stmt === false) {
    ...
    $stmtQ = $conn->prepare("SELECT quiz1  FROM userslog WHERE username = ?");
    if ($stmtQ === false) {
        die("Insert statement preparation failed: " . htmlspecialchars($conn->error));
    }
   ...
   $stmtQ -> execute();
   $stmtQ->bind_result($q1);


   if ($stmtQ->fetch()) {
    // Use the fetched value
    echo "Fetched value: " . $q1;
    } else {
    ...
    //stmt1 checks if mo of user is null
                ...
                if ($stmt1->num_rows > 0) {
                    ...
                    $stmt2 = $conn->prepare("UPDATE userslog SET mo = COALESCE((quiz1 + ?) / 2, ?) WHERE username = ?");
                    if($stmt2===false){
    ...
    if($stmt->execute()===TRUE){
        ...
        echo "Error executing query: " . htmlspecialchars($stmtQ->error) . "<br>";
    }
